...
class InstantProvider extends AbstractInstantProvider
{
    ...
    /**
     * @param \Mirasvit\BlogMx\Model\Post $model
     */
    private function mapItem($model, int $storeId): array
    {
        $om = ObjectManager::getInstance();

        /** @var \Mirasvit\BlogMx\ViewModel\PostViewModel $viewModel */
        $viewModel = $om->get('Mirasvit\BlogMx\ViewModel\PostViewModel');

        return [
            'name' => $model->getName(),
            'url'  => $viewModel->getUrl($model),
        ];
    }
