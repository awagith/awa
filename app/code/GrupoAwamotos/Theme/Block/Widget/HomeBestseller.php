...
use Rokanthemes\BestsellerProduct\Block\Widget\Bestsellerproduct;

class HomeBestseller extends Bestsellerproduct
{
    private const DATA_MAP = [
        'title' => ['title'],
        'qty' => ['qty', 'limit'],
        'row_show' => ['row_show', 'row'],
    ...
    {
        return max(1, (int) $this->getConfig('row_show'));
    }
    ...
    {
        $domId = (string) ($this->getData('dom_id') ?: 'awa-home-bestseller');

        return preg_replace('/[^a-zA-Z0-9_-]+/', '-', $domId) ?: 'awa-home-bestseller';
    }

    public function getCarouselVariant(): string
    {
        return trim((string) ($this->getData('carousel_variant') ?: 'awa-home-merch-shelf--bestseller'));
    }
