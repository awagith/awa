...
|-------|---------|--------|
| **Awa** | `@Awa` | Agente principal — implementa módulos Magento 2 |
| **Implementador** | `@Implementador` | Implementa features completas com código real |
| **Revisor** | `@Revisor` | Revisa código sem modificar |
| **Arquiteto** | `@Arquiteto` | Planeja antes de implementar |
| **Debugger** | `@Debugger` | Diagnostica causa raiz antes de corrigir |

### .github/instructions/ (4 Instruction Files)

| Arquivo | Aplica a | Função |
|---------|----------|--------|
| magento-database | `db_schema.xml`, `Setup/`, `ResourceModel/` | Regras para Declarative Schema e ORM |
| services-api | `Model/`, `Api/`, `Helper/` | Regras para Services, APIs e Helpers |
| phtml-templates | `*.phtml` | Regras para templates PHTML |
| tests | `Test/`, `tests/` | Regras para PHPUnit |
