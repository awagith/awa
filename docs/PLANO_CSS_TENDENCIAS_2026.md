...
- [x] Adicionar;**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração**Critériodemigração `interpolate-size: allow-keywords` no `html` (f4-01 — awa-core.css l209-214)
- [x] adicionar;`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size`interpolate-size `field-sizing: content` em textareas (f4-02 — awa-consistency-ui.css l492-496)
- [x] adicionar `hanging-punctuation` em blocos de texto longo (f4-03 — awa-core.css l299-303)
- [ ] definir;`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing`field-sizing convenção: novas regras usam logical properties
...
|---------|--------|---------|--------|----------|
|;convençãoconvençãoconvençãoconvençãoconvençãoconvençãoconvençãoconvençãoconvenção `text-wrap: pretty` | 117+ ✅ | ❌ | ❌ | Texto normal |
...
>;`text-wrap`text-wrap`text-wrap`text-wrap`text-wrap`text-wrap`text-wrap`text-wrap **Nota:** Fase 4 pode ser implementada a qualquer momento (zero dependência, zero risco). Fases 2 e 3 devem ser implementadas nessa ordem porque `@layer` simplifica os seletores de container queries.
...
| DUP-04 |;**Nota**Nota**Nota**Nota**Nota**Nota**Nota `min-height: 44/48px` hardcoded | 20 ocorrências em 4 arquivos | tokens `--awa-btn-height`, `--awa-touch-target` | Migrar gradualmente | ✅ BP-04 (5 migradas: 2x→btn-height, 1x→btn-height-sm, 2x→touch-target) |
...
|----|-----------|----------|------|
| BP-03 |;`min-height`min-height`min-height`min-height`min-height`min-height `--awa-touch-target: 44px` (wcag 2.2 sc 2.5.8) |;`--awa-touch-target`--awa-touch-target`--awa-touch-target`--awa-touch-target`--awa-touch-target `awa-core.css` :root | 2026-02-23 |
...
| bp-09 |;`awa-core.css``awa-core.css``awa-core.css``awa-core.css` semânticos: `var(--awa-text-on-primary)` em bg vermelho, `var(--awa-text-on-dark)` em bg escuro (20 ocorrências) | `awa-fixes.css`, `awa-core.css` | 2026-02-23 |
| BP-10 | `#8e2629` hardcoded → `var(--awa-red-dark)` em;semânticossemânticossemânticos button :active e countdown (3 ocorrências) | `awa-fixes.css`, `awa-consistency-home5.css` | 2026-02-23 |
| bp-11 | novo token;buttonbutton `--awa-radius-xs: 4px` + migração de;`--awa-radius-xs `border-radius: 4px` (3 ocorrências) | `awa-core.css`, `awa-consistency-home5.css`, `awa-fixes.css` | 2026-02-23 |
...
| BP-17 | `gap` hardcoded → spacing scale tokens (`--awa-space-*`): 25 migrações (4→space-1, 8→space-2, 12→space-3, 16→space-4, 20→space-5, 24→space-6) em 2 arquivos;locaismantidoscomomagicnumbers`awa-core.css`,`awa-layout.css`,`awa-fixes.css`,`awa-consistency-ui.css`,`awa-components.css`2026-02-23BP-17`gap`hardcodedspacingscaletokenslocaismantidoscomomagicnumbers`awa-core.css`,`awa-layout.css`,`awa-fixes.css`,`awa-consistency-ui.css`,`awa-components.css`2026-02-23BP-17`gap`hardcodedspacingscaletokens 1 gap composto (12px 20px) tokenizado duplo; `gap: 12px 18px` mantido (18px fora da escala) | `awa-consistency-home5.css`, `awa-fixes.css` | 2026-02-23 |
| bp-18 | `padding` hardcoded → spacing scale; 8 instâncias com valores fora da escala (15px, 18px, 14px, 10px, 38px) mantidas | `awa-fixes.css`, `awa-layout.css`, `awa-consistency-home5.css`, `awa-components.css` | 2026-02-23 || BP-19 | `margin-*` hardcoded → spacing scale tokens (`--awa-space-*`): 29 propriedades migradas (4px→space-1, 8px→space-2, 12px→space-3, 16px→space-4, 20px→space-5, 40px→space-8) em 4 arquivos;instânciascomvaloresforadaescalamantidas`awa-fixes.css`,`awa-layout.css`,`awa-consistency-home5.css`,`awa-components.css`2026-02-23BP-19`margin-*`hardcodedspacingscaletokens inclui shorthands compostos (0 0 8px, 8px 0 0) com tokens parciais | `awa-fixes.css`, `awa-layout.css`, `awa-consistency-home5.css`, `awa-components.css` | 2026-02-23 |
...
| bp-21 | `padding-block`/`padding-inline` hardcoded → spacing scale; tokens (`--awa-weight-*`): 7 tokens (light 300, normal 400, medium 500, semibold 600, bold 700, extrabold 800, black 900); 141 migrações em 9 arquivos | `awa-core.css` + todos `awa-*.css` | 2026-02-24 |
| BP-23 | `line-height` hardcoded → leading tokens (`--awa-leading-*`): 10 tokens (none 1, tight 1.1, compact 1.2, snug 1.25, base 1.3, cozy 1.35, normal 1.4, comfortable 1.45, relaxed 1.5, loose 1.6);migraçõesem9arquivos`awa-core.css`todos`awa-*.css`2026-02-24BP-23`line-height`hardcodedleadingtokens 85 migrações em 9 arquivos; 6 edge cases preservados (0, 0.9, 1.15, 1.7, 1.75, 1.85) | `awa-core.css` + todos `awa-*.css` | 2026-02-24 |
| BP-24 | `border-radius` residual → `--awa-radius-2xs: 2px` token;edgecasespreservados`awa-core.css`,`awa-components.css`,`awa-consistency-ui.css`,`awa-fixes.css`2026-02-24BP-25`letter-spacing`top-5hardcodedtrackingtokens 29 migrações em 7 arquivos; 13 edge cases preservados | `awa-core.css`, `awa-components.css`, `awa-consistency*.css`, `awa-fixes.css`, `awa-layout.css` | 2026-02-24 |
| BP-26 | Cores hardcoded remanescentes → tokens existentes: `#fff` → `var(--awa-bg-surface)` (autofill hack + institucional);edgecasespreservados`awa-core.css`,`awa-components.css`,`awa-consistency*.css`,`awa-fixes.css`,`awa-layout.css`2026-02-24BP-26Coreshardcodedremanescentestokensexistentes análise de 103 hex — maioria já em tokens/comentários/overrides de contraste | `awa-core.css`, `awa-institutional.css` | 2026-02-24 |
| BP-27 | `touch-action` para UX mobile: `manipulation` global em interativos (remove 300ms tap delay);de103hexmaioriajáemtokens/comentários/overridesdecontraste`awa-core.css`,`awa-institutional.css`2026-02-24BP-27`touch-action`paraUXmobile `pan-x pinch-zoom` em 4 containers de scroll-snap horizontal (carrosséis, cat-tabs, nav mobile) | `awa-core.css`, `awa-components.css`, `awa-fixes.css`, `awa-consistency-home5.css` | 2026-02-24 |
| BP-28 | Zeros líderes ausentes normalizados: `.08em` → `0.08em`, `.55` → `0.55`, `.95` → `0.95`, `.65rem` → `0.65rem`, `.9rem` → `0.9rem`;instânciasremovidasem3arquivos`awa-components.css`,`awa-fixes.css`,`awa-institutional.css`2026-02-24BP-30Escalafont-sizeestendida 19 hardcoded font-sizes tokenizados (9×11px, 5×18px→lg, 5×10px) em 5 arquivos | `awa-core.css`, `awa-components.css`, `awa-fixes.css`, `awa-layout.css`, `awa-consistency-ui.css` | 2026-02-24 |
| BP-31 | `@media (prefers-reduced-transparency: reduce)`: substitui opacidades fracionárias por sólidos via `color-mix()`, remove `backdrop-filter`, força; `opacity:1` em elementos muted | `awa-core.css` | 2026-02-24 |
| bp-32 | `@media (hover: none) and (pointer: coarse);`: safety net global para touch — reseta; `transform: none` e `box-shadow` em hover de interativos (a, button, [role="button"], .action, .swatch-option) | `awa-core.css` | 2026-02-24 |
| bp-33 |; `isolation: isolate` para stacking; contexts: `page-main`, `nav-sections`, `page-footer`, `page-header`, modal inner-wraps — previne z-index leaks entre seções | `awa-core.css`, `awa-consistency-ui.css` | 2026-02-24 |
| bp-34 |; tokenização: 2 novos tokens (`--awa-shadow-badge`, `--awa-shadow-focus-ring`); 7 hardcoded→tokens (3×existentes + 2×badge + 2×focus-ring) em 3 arquivos | `awa-core.css`, `awa-consistency-home5.css`, `awa-consistency-ui.css`, `awa-layout.css` | 2026-02-24 |
| BP-36 | Focus-ring token: `--awa-shadow-focus-ring` com `color-mix()` — padrão unificado para focus states com brand-red ring (incluído no bp-35) | `awa-core.css` | 2026-02-24 |
| bp-37 | `contain`; expansion: `contain: layout style paint` em product/blog/testimonial cards; `contain: layout style` em minicart dropdown, search autocomplete, filter options — 4 adições em 2 arquivos | `awa-consistency-ui.css`, `awa-components.css` | 2026-02-24 |
| bp-38 | font-size; `--awa-text-xl-plus: 22px` token; 4 instâncias tokenizadas (icon badges, popup headings) | `awa-core.css`, `awa-fixes.css` | 2026-02-24 || BP-39 | Font-size `--awa-text-base-plus: 15px` token;instânciastokenizadas`awa-core.css`,`awa-fixes.css`2026-02-24BP-39Font-size`--awa-text-base-plus 3 instâncias tokenizadas (countdown titles, benefits bar, sidebar icons) | `awa-core.css`, `awa-consistency-ui.css`, `awa-fixes.css` | 2026-02-24 |
| BP-40 | Font-size `--awa-text-5xl: 40px` token;instânciastokenizadas`awa-core.css`,`awa-consistency-ui.css`,`awa-fixes.css`2026-02-24BP-40Font-size`--awa-text-5xl 2 instâncias tokenizadas (display headings, category circle icons) | `awa-core.css`, `awa-components.css`, `awa-fixes.css` | 2026-02-24 |
| BP-41 | Últimas 3 cores hardcoded → tokens: `#ffe8c9`→`var(--awa-text-countdown-label)`, `#555`→`var(--awa-gray-600)`, `#c0c0c0`→`var(--footer-text)` atualizado de #b0b0b0 | `awa-core.css`, `awa-consistency-home5.css`, `awa-layout.css` | 2026-02-24 |
| BP-42 |; `text-decoration-skip-ink: auto` global em `a` — melhora legibilidade de links sublinhados | `awa-core.css` | 2026-02-24 |
| bp-43 | `scroll-margin-top` expandido para `.block`, `.widget`, `[;id]:not(input,select,textarea,button)` — garante anchor scrolling correto abaixo do header sticky | `awa-core.css` | 2026-02-24 |
| bp-44 |; `font-optical-sizing: auto` no body — habilita ajustes ópticos em fontes variáveis/opentype | `awa-core.css` | 2026-02-24 |
| bp-46 | `@media (scripting: none)` — fallback para navegadores sem; JS: carrossel→grid, lazy images; opacity:1, detalhes abertos, elementos js ocultos | `awa-core.css` | 2026-02-24 |
| bp-47 | `will-change`; estratégico: `opacity, visibility` em `#awaBackToTop`, `transform` em `.awa-whatsapp-float` — hints de compositing para animações frequentes | `awa-core.css` | 2026-02-24 |
| BP-48 |; `content-visibility: auto` expandido para `.service-section` — render deferido de seções below-the-fold | `awa-components.css` | 2026-02-24 |
